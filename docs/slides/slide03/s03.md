...
2. 对于每个区分器$D$（原文：For each PPT distinguisher $D$，PPT是指...？），均存在一个可忽略的函数$negl$，使得 $\lvert Pr[D(r) = 1] - Pr[D(G(s)) = 1] \rvert \leq negl(n)$（其中$r$从$\{ 0, 1\}^{l(n)}$中均匀地随机选择，$s$从$\{ 0, 1 \}^{n}$中均匀地随机选择。
